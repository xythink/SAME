...
def get_shadow_node_model(
    ...
    """
    if n_classes is None:
        n_classes = nclasses_dict[dataset] + 1
...
def get_ood_detector(model: str="conv3",x_dim=10):
    '''
    Return the model used to detect ood query.
    '''
    if model in ["vgg16", "res18"]:
        net = model_dict[model](pretrained=False)
        in_feat = net.fc.in_features
        net.fc = nn.Linear(in_feat, 2)
    elif model in ["mlp"]:
        net = model_dict[model](in_dim=x_dim)
    else:
